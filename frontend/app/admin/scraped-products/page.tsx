...
type SourceSite = 'myntra' | 'footlocker' | 'vegnonveg' | 'limitededt' | 'superkicks' | 'nike';
...
export default function ScrapedProductsPage() {
  ...
  async function handleRunScraper() {
    ...
    try {
      ...
      showToast('Scraper triggered via GitHub Actions');
    } catch (err) {
      showToast((err as Error).message);
    } finally {
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 text-xs font-semibold bg-white text-zinc-900 rounded-lg hover:bg-zinc-100 disabled:opacity-50 transition-colors"
        >
          {runningCron ? (
