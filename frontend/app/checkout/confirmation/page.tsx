...
function PaidState({ orderNumber }: { orderNumber: string }) {
  return (
    <>
      ...
      <p className="text-xs font-bold tracking-[0.3em] uppercase text-zinc-400 mb-1">Payment Confirmed</p>
      <h1 className="text-2xl font-black tracking-tight text-zinc-900 mb-1">You&apos;re all set!</h1>
      <p className="text-sm text-zinc-500 mb-8">
        Payment for order <span className="font-bold text-zinc-900">{orderNumber}</span> has been confirmed.
        We&apos;re now processing your order.
      </p>
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5 mb-6 text-left">
        <p className="text-xs font-bold tracking-widest uppercase text-emerald-800 mb-2">What&apos;s next?</p>
        <ul className="space-y-2 text-xs text-emerald-800">
          ...
          <li className="flex items-start gap-2">
            ...
            <span>Delivery in 3–7 business days. Track your order in My Orders.</span>
          </li>
        </ul>
      </div>
    </>
...
function PendingState({ orderNumber, total }: { orderNumber: string; total: number }) {
...
function FailedState({ orderNumber, total }: { orderNumber: string; total: number }) {
  const waText = encodeURIComponent(`Hi! My online payment failed for order ${orderNumber}. I'd like to pay via UPI instead.`);
  const waUrl = `https://wa.me/${STORE_WA}?text=${waText}`;
  ...
  return (
    <>
      ...
      <p className="text-xs font-bold tracking-[0.3em] uppercase text-red-400 mb-1">Payment Failed</p>
      <h1 className="text-2xl font-black tracking-tight text-zinc-900 mb-1">Something went wrong</h1>
      <p className="text-sm text-zinc-500 mb-6">
        Your payment for order <span className="font-bold text-zinc-900">{orderNumber}</span> could not be processed.
        Your order is saved — you can pay via UPI instead.
      </p>

      <div className="bg-red-50 border border-red-200 rounded-xl p-5 mb-6 text-left">
        <p className="text-xs font-bold tracking-widest uppercase text-red-800 mb-2">Pay manually via UPI</p>
        <div className="bg-white border border-red-100 rounded-lg p-4 text-center mb-3">
          <p className="text-xs text-zinc-500 mb-1">Pay to UPI ID</p>
          <p className="text-lg font-black text-zinc-900 tracking-wider">{UPI_ID}</p>
          <p className="text-2xl font-black text-zinc-900 mt-1">₹{total.toLocaleString('en-IN')}</p>
        </div>
        <p className="text-xs text-red-700">After paying, send the screenshot on WhatsApp with order number <strong>{orderNumber}</strong>.</p>
      </div>
...
function ConfirmationContent() {
  ...
  const paymentStatus = (params.get('paymentStatus') || 'pending') as PaymentStatus;
