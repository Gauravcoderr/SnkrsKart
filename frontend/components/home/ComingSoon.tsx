        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {products.slice(0, 8).map((product) => (
              <div
                key={product.id}
                className="relative bg-white rounded-xl overflow-hidden shadow-md border border-zinc-100"
              >
                <ProductCard product={product} />
                {/* Transparent overlay intercepts clicks before Link navigates */}
                <button
                  type="button"
                  aria-label={`See drop details for ${product.name}`}
